...
MPIising::MPIising(int argc, char **argv)
{
    if(argc != 7) {
        cout << "Bad usage! Wrong number of input parameters!" << endl;
        cout << "Usage: mpirun -hostfile <hostfilename> -n <processors> ./<executable> nSpins msc initTemp finalTemp tempStep outputFileName" << endl;
        exit(0);
    }

    this->n_spins = atoi(argv[1]);
    this->mcs = atoi(argv[2]);
    this->initial_temp = atof(argv[3]);
    this->final_temp = atof(argv[4]);
    this->temp_step = atof(argv[5]);
    this->outfilename = argv[6];
...
void MPIising::initMPI(int argc, char **argv) {
    //  MPI initializations
    MPI_Init (&argc, &argv);
